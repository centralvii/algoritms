...
1. [Алгоритмы и объекты](#algs);
  ...
  + [Визуализация алгоритмов](#visualalg);
2. [Алгоритмы и параметры](#algsparams);
  + [Параметрическое управление алгоритмами](#algupr);
  + [Системные параметры алгоритмов](#sysparam);
  + [Объектные параметры алгоритмов](#objparam);
  + [Проверка и типизация параметров](#checkparam);
  + [Проверка и шаблоны PCRE](#symbols);

# <a name="algs"></a> 1. Алгоритмы и объекты
...
## <a name="visualalg"></a> Визуализация алгоритмов

Зачем!? - формальность или здравый смысл

ГОСТ 19.701-90 

![alt](https://i.imgur.com/yancQAG.png)

### Сервисы визуализации

1. Microsoft Visio - зависимость от версии и комплектности раб. места
2. Figma - "автономность" связанных элементов
3. Построитель - https://app.diagrams.net/

Формат:
- Скрины
- XML файл

![alt](https://i.imgur.com/5tD2kko.png)

```
def n:= 10 // просто 10 для цикла `for`
def r := new Random() // генерация числа
def my_sum := 0 // мое
def res_str := "" // строка

for (def i:=0; i < n; i++) {
    my_num := r.nextInt(10) // результат вызова метода, сгенерировать из метода `r` следующее целое число в диапазоне от `0 до 10`
    res_str := res_str + my_num // вся информация из `my_num` приклеивается справа

    printout (res_str) // отладка алгоритма в лог
}

return res_str // итог после завершения цикла
```
Как будет выглядить этот алгоритм в виде диаграммы?

![alt](https://i.imgur.com/Tbwbh1x.png)


# <a name="algsparams"></a> 2. Алгоритмы GD и параметры

## <a name="algupr"></a> Параметрическое управление алгоритмами

Кнопки `alg` `algWithParams`

![alt](https://i.imgur.com/7TBCove.png)

Фактические параметры - подставляемые значения при вызове (вне алгоритма) <br/>
Формальные параметры - идентификаторы входящих данных/объектов (доступны изнутри алгоритма)

## <a name="sysparam"></a> Системные параметры алгоритма

Параметры проектирования - определяют возможности алгоритма <br/>
`Сопоставление классу`  <br/>
`Опция возврата null значения`  <br/>
![alt](https://i.imgur.com/dHWWAub.png) <br/>
Параметры запуска - определают доступ к реальным данным  <br/>
`/#/card/846503/type/846495`
![image](https://github.com/user-attachments/assets/d01e15cf-86ce-42e4-b23b-877ab6d30f0a)

![image](https://github.com/user-attachments/assets/c9447cdb-c03b-442f-b328-5fa8701b4ef4)

## <a name="objparam"></a> Объектные параметры алгоритма

`alg(Алгоритм, параметр_объект: ob)` <br/>

Передача стандартных структур:
- ArrayList
- HashSet
- HashMap... и др

Передача пользовательских объектов:
- В формате любого типа объекта. Кнопки `getObject` `param`

Алг_расшифровки_объекта <br/>
```
def ob:=getObject(параметр_объект, Пиломатериал, Наименование, Спецификация, Влажность)

return ob.Наименование + " " + ob.Спецификация
```
```
def co := currentObject()

def r := alg(Алг_расшифровки_объекта, параметры_пм: co)

printout(r) 
```

## <a name="checkparam"></a> Проверка и типизация параметров

null - пустое техническое значение, аварийная пустота, означает что параметр не пришел, не заполнился, потерялся. <br/>
<br/>
Первое что нужно сделать в алгоритмах, сделать проверку на null `isNull`, `isNotNull`, `not`<br/>
<br/>
Проверки на НЕпустое (состояние)
```
if (not isNull(a)) {...}
if (! isNull(a)){...}
if (isNotNull(a)){...}
...
## <a name="symbols"></a> Проверка и шаблоны PCRE

1. Метасимволы и диапазоны
```
^ начало данных    $ конец строки
\d любая цифра     \s пробельный символ
\w символ слова    \t табуляция
[0-9] цифровой диапазон [a-za-я] буквы

def mask := '[0-9]{11}'

if (into.matches(mask)) {
...
```
\ перед значимым символом
```
Чтобы передать параметр - в алг. создаем параметр объекта  <br/>
через кнопка `param` задается параметр <br/>
Технические символы (типо скобочки), в маске нужно дикларировать ковычками \\ <br/>
`matches` - проверка на сходство
Пример:
```
// шаблон телефона
def phone:='+7(342)123-45-67'

def r:=alg(Алг_проверки_номера, параметр_номер (#номер(сам генерится)): phone)

printout(r)
```
```
// алг. проверки номера
def p:=параметр_номер
def res:=""

def mask:="7\\([0-9]{3}\\)[0-9]{3}-[0-9]{2}-[0-9]{2}"

if (p.matches(mask)) {
  res:="Можно звонить на" + p
} else {
  res:="Нельзя звонить! Номер" + p + "неккоректен Ошибка"
}
res
```
